...
/// Build version for generated midas crate
const BUILD_VERSION: u32 = 0; // TODO: bump this
...
impl Version {
    ...
    /// Return version for crate version (e.g., "29.1.1")
...
impl ApiDefinition {
    ...
    pub fn from_file<P: AsRef<std::path::Path>>(path: P) -> Result<Self> {
        ...
        let mut api_def: ApiDefinition =
            serde_json::from_str(&content).map_err(SchemaError::JsonParse)?;
    ...
    pub fn validate(&self) -> Result<()> {
        for (name, method) in &self.rpcs {
            ...
            // Check for argument ordering special cases
    ...
    fn validate_argument_ordering(&self, _method_name: &str, method: &BtcMethod) -> Result<()> {
        ...
        // Note: These special cases are automatically fixed during code generation
        for i in 0..args.len() - 1 {
            if !args[i].required && args[i + 1].required {
                break; // Only check once per method
            }
        }
